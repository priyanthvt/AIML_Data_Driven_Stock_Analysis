...
if selected_page == 'Volatility Analysis':
    ...
    mask = correlation_df < 0.5
    fig, ax = plt.subplots(figsize=(30, 25))
    sns.heatmap(correlation_df, mask = mask, annot = True)
    st.pyplot(fig)

elif selected_page == 'Strong Positive Correlation Stocks':
    cursor.execute("SELECT * FROM POSITIVE_CORRELATION ORDER BY CORRELATION DESC")
    rows = cursor.fetchall()

    df = pd.DataFrame(rows, columns = ['TICKER 1', 'TICKER 2', 'CORRELATION COEFFICIENT'])

    st.dataframe(df)

elif selected_page == 'Negative Correlation Stocks':
    cursor.execute("SELECT * FROM NEGATIVE_CORRELATION ORDER BY CORRELATION")
    rows = cursor.fetchall()
